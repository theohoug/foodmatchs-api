const express = require('express');
const Database = require('better-sqlite3');
const path = require('path');
const { v4: uuidv4 } = require('uuid');
const { authenticateToken, optionalAuth } = require('../middleware/auth');

const router = express.Router();
const DB_PATH = path.join(__dirname, '..', 'database', 'foodmatchs.db');
const getDb = () => new Database(DB_PATH);

// =====================================================
// GET QUESTIONS FOR QUIZ (no auth required for onboarding)
// =====================================================
router.get('/questions', (req, res) => {
    const db = getDb();
    try {
        const { count = 15, category } = req.query;
        
        let query = 'SELECT * FROM questions WHERE 1=1';
        const params = [];
        
        if (category) {
            query += ' AND category = ?';
            params.push(category);
        }
        
        query += ' ORDER BY RANDOM() LIMIT ?';
        params.push(parseInt(count));
        
        const questions = db.prepare(query).all(...params);
        db.close();
        
        res.json(questions);
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// GET ALL PROFILES (for display)
// =====================================================
router.get('/profiles', (req, res) => {
    const db = getDb();
    try {
        const profiles = db.prepare('SELECT * FROM profiles ORDER BY name').all();
        db.close();
        res.json(profiles);
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// CALCULATE PROFILE FROM ANSWERS (no auth - for onboarding)
// =====================================================
router.post('/calculate-profile', (req, res) => {
    const db = getDb();
    try {
        const { answers } = req.body; // [{ question_id, liked: true/false }]
        
        if (!answers || !Array.isArray(answers)) {
            db.close();
            return res.status(400).json({ error: 'Answers array required' });
        }
        
        // Get questions for liked answers
        const likedIds = answers.filter(a => a.liked).map(a => a.question_id);
        
        if (likedIds.length === 0) {
            // Default profile
            const defaultProfile = db.prepare('SELECT * FROM profiles LIMIT 1').get();
            db.close();
            return res.json({ profile: defaultProfile, score: 0 });
        }
        
        // Count tag occurrences
        const tagCounts = {};
        for (const qId of likedIds) {
            const question = db.prepare('SELECT tags FROM questions WHERE id = ?').get(qId);
            if (question && question.tags) {
                const tags = question.tags.split(',');
                for (const tag of tags) {
                    tagCounts[tag.trim()] = (tagCounts[tag.trim()] || 0) + 1;
                }
            }
        }
        
        // Get all profiles and score them
        const profiles = db.prepare('SELECT * FROM profiles').all();
        let bestProfile = profiles[0];
        let bestScore = 0;
        
        for (const profile of profiles) {
            if (profile.tags) {
                const profileTags = profile.tags.split(',');
                let score = 0;
                for (const tag of profileTags) {
                    score += tagCounts[tag.trim()] || 0;
                }
                if (score > bestScore) {
                    bestScore = score;
                    bestProfile = profile;
                }
            }
        }
        
        db.close();
        res.json({ profile: bestProfile, score: bestScore });
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// SUBMIT QUIZ ANSWERS & GET PROFILE (with auth)
// =====================================================
router.post('/submit', authenticateToken, (req, res) => {
    const db = getDb();
    try {
        const { answers } = req.body;
        const userId = req.user.userId;
        
        if (!answers || !Array.isArray(answers)) {
            db.close();
            return res.status(400).json({ error: 'Answers array required' });
        }
        
        // Save answers
        const insertAnswer = db.prepare(`
            INSERT OR REPLACE INTO user_answers (user_id, question_id, liked)
            VALUES (?, ?, ?)
        `);
        
        for (const ans of answers) {
            insertAnswer.run(userId, ans.question_id, ans.liked ? 1 : 0);
        }
        
        // Calculate profile based on liked tags
        const likedAnswers = db.prepare(`
            SELECT q.tags FROM user_answers ua
            JOIN questions q ON ua.question_id = q.id
            WHERE ua.user_id = ? AND ua.liked = 1
        `).all(userId);
        
        // Count tag occurrences
        const tagCounts = {};
        for (const row of likedAnswers) {
            if (row.tags) {
                const tags = row.tags.split(',');
                for (const tag of tags) {
                    tagCounts[tag.trim()] = (tagCounts[tag.trim()] || 0) + 1;
                }
            }
        }
        
        // Get all profiles and score them
        const profiles = db.prepare('SELECT * FROM profiles').all();
        let bestProfile = profiles[0];
        let bestScore = 0;
        
        for (const profile of profiles) {
            if (profile.tags) {
                const profileTags = profile.tags.split(',');
                let score = 0;
                for (const tag of profileTags) {
                    score += tagCounts[tag.trim()] || 0;
                }
                if (score > bestScore) {
                    bestScore = score;
                    bestProfile = profile;
                }
            }
        }
        
        // Save user profile
        if (bestProfile) {
            db.prepare(`
                INSERT OR REPLACE INTO user_profiles (user_id, profile_id, score)
                VALUES (?, ?, ?)
            `).run(userId, bestProfile.id, bestScore);
        }
        
        // Award XP for completing quiz
        const xpGain = 50;
        db.prepare('UPDATE users SET total_xp = total_xp + ? WHERE id = ?').run(xpGain, userId);
        
        db.close();
        
        res.json({
            profile: bestProfile,
            score: bestScore,
            xp_gained: xpGain
        });
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// GET USER PROFILE
// =====================================================
router.get('/profile', authenticateToken, (req, res) => {
    const db = getDb();
    try {
        const userId = req.user.userId;
        
        const userProfile = db.prepare(`
            SELECT p.*, up.score FROM user_profiles up
            JOIN profiles p ON up.profile_id = p.id
            WHERE up.user_id = ?
            ORDER BY up.assigned_at DESC LIMIT 1
        `).get(userId);
        
        db.close();
        res.json(userProfile || null);
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// DAILY QUIZ - GET TODAY'S MENU
// =====================================================
router.get('/daily', authenticateToken, (req, res) => {
    const db = getDb();
    try {
        const userId = req.user.userId;
        const today = new Date().toISOString().split('T')[0];
        
        // Check if already answered today
        const existing = db.prepare(`
            SELECT * FROM daily_quiz WHERE user_id = ? AND DATE(created_at) = ?
        `).get(userId, today);
        
        if (existing) {
            // Get meals - BEFORE closing db
            const starter = existing.starter_id ? db.prepare('SELECT * FROM meals WHERE id = ?').get(existing.starter_id) : null;
            const main = existing.main_id ? db.prepare('SELECT * FROM meals WHERE id = ?').get(existing.main_id) : null;
            const dessert = existing.dessert_id ? db.prepare('SELECT * FROM meals WHERE id = ?').get(existing.dessert_id) : null;
            const cheese = existing.cheese_id ? db.prepare('SELECT * FROM meals WHERE id = ?').get(existing.cheese_id) : null;
            const wine = existing.wine_id ? db.prepare('SELECT * FROM meals WHERE id = ?').get(existing.wine_id) : null;
            
            const alternatives = JSON.parse(existing.alternatives_json || '{}');
            
            db.close();
            return res.json({
                already_answered: true,
                menu: { starter, main, dessert, cheese, wine },
                alternatives
            });
        }
        
        db.close();
        res.json({ already_answered: false, needs_quiz: true });
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// DAILY QUIZ - SUBMIT PREFERENCES
// =====================================================
router.post('/daily', authenticateToken, (req, res) => {
    const db = getDb();
    try {
        const userId = req.user.userId;
        const { budget = 'medium', servings = 2, mood, include_cheese, include_wine } = req.body;
        
        // Check if already answered today
        const today = new Date().toISOString().split('T')[0];
        const existing = db.prepare(`
            SELECT id FROM daily_quiz WHERE user_id = ? AND DATE(created_at) = ?
        `).get(userId, today);
        
        if (existing) {
            db.close();
            return res.status(400).json({ error: 'Déjà généré aujourd\'hui' });
        }
        
        // Get user preferences
        const prefs = db.prepare('SELECT * FROM user_preferences WHERE user_id = ?').get(userId);
        const diet = prefs?.diet || 'omnivore';
        
        // Build query conditions
        let conditions = [];
        if (diet === 'vegetarian') conditions.push('is_vegetarian = 1');
        if (diet === 'vegan') conditions.push('is_vegan = 1');
        if (budget && budget !== 'all') conditions.push(`budget = '${budget}'`);
        
        const whereClause = conditions.length > 0 ? 'AND ' + conditions.join(' AND ') : '';
        
        // Get random meals
        const getRandomMeal = (type) => {
            const meal = db.prepare(`
                SELECT * FROM meals WHERE type = ? ${whereClause}
                ORDER BY RANDOM() LIMIT 1
            `).get(type);
            
            // Fallback without conditions if nothing found
            if (!meal) {
                return db.prepare(`
                    SELECT * FROM meals WHERE type = ?
                    ORDER BY RANDOM() LIMIT 1
                `).get(type);
            }
            return meal;
        };
        
        const starter = getRandomMeal('starter');
        const main = getRandomMeal('main');
        const dessert = getRandomMeal('dessert');
        const cheese = include_cheese ? getRandomMeal('cheese') : null;
        const wine = include_wine ? getRandomMeal('wine') : null;
        
        // Get alternatives
        const getAlternatives = (type, excludeId) => {
            return db.prepare(`
                SELECT * FROM meals WHERE type = ? AND id != ?
                ORDER BY RANDOM() LIMIT 3
            `).all(type, excludeId || 'none');
        };
        
        const alternatives = {
            starters: starter ? getAlternatives('starter', starter.id) : [],
            mains: main ? getAlternatives('main', main.id) : [],
            desserts: dessert ? getAlternatives('dessert', dessert.id) : []
        };
        
        // Save daily quiz
        const quizId = uuidv4();
        
        db.prepare(`
            INSERT INTO daily_quiz (id, user_id, budget, servings, mood, starter_id, main_id, dessert_id, cheese_id, wine_id, alternatives_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
            quizId, 
            userId, 
            budget || 'medium', 
            servings || 2, 
            mood || null, 
            starter?.id || null, 
            main?.id || null, 
            dessert?.id || null, 
            cheese?.id || null, 
            wine?.id || null, 
            JSON.stringify(alternatives)
        );
        
        // Update streak
        const streak = db.prepare('SELECT * FROM streaks WHERE user_id = ?').get(userId);
        const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
        
        let newStreak = 1;
        if (streak) {
            if (streak.last_quiz_date === yesterday) {
                newStreak = streak.current_streak + 1;
            } else if (streak.last_quiz_date === today) {
                newStreak = streak.current_streak;
            }
            
            const longestStreak = Math.max(streak.longest_streak || 0, newStreak);
            db.prepare(`
                UPDATE streaks SET current_streak = ?, longest_streak = ?, last_quiz_date = ?
                WHERE user_id = ?
            `).run(newStreak, longestStreak, today, userId);
        } else {
            // Create streak record
            db.prepare(`
                INSERT INTO streaks (user_id, current_streak, longest_streak, last_quiz_date)
                VALUES (?, ?, ?, ?)
            `).run(userId, 1, 1, today);
        }
        
        // Award XP
        const xpGain = 25 + (newStreak > 1 ? 5 * Math.min(newStreak, 10) : 0);
        db.prepare('UPDATE users SET total_xp = total_xp + ? WHERE id = ?').run(xpGain, userId);
        
        db.close();
        
        res.json({
            menu: { starter, main, dessert, cheese, wine },
            alternatives,
            streak: newStreak,
            xp_gained: xpGain
        });
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

// =====================================================
// SWAP MEAL IN MENU
// =====================================================
router.post('/daily/swap', authenticateToken, (req, res) => {
    const db = getDb();
    try {
        const userId = req.user.userId;
        const { meal_type, new_meal_id } = req.body;
        const today = new Date().toISOString().split('T')[0];
        
        const columnMap = {
            starter: 'starter_id',
            main: 'main_id',
            dessert: 'dessert_id',
            cheese: 'cheese_id',
            wine: 'wine_id'
        };
        
        const column = columnMap[meal_type];
        if (!column) {
            db.close();
            return res.status(400).json({ error: 'Invalid meal type' });
        }
        
        db.prepare(`
            UPDATE daily_quiz SET ${column} = ?
            WHERE user_id = ? AND DATE(created_at) = ?
        `).run(new_meal_id, userId, today);
        
        const newMeal = db.prepare('SELECT * FROM meals WHERE id = ?').get(new_meal_id);
        
        db.close();
        res.json({ success: true, meal: newMeal });
        
    } catch (error) {
        db.close();
        res.status(500).json({ error: error.message });
    }
});

module.exports = router;
